...
class TestFindlog(object):
    ...
    def test_search_method_using_or(self):
        parser = logfind.return_parser()
        parsed_arguments = parser.parse_args(self.arguments_or)
        assert_true(logfind.search_method(parsed_arguments, 'this is a test'))

    def test_search_method_using_or_fail(self):
        parser = logfind.return_parser()
        parsed_arguments = parser.parse_args(self.arguments_or)
        assert_false(logfind.search_method(parsed_arguments, 'blah blah blah'))

    def test_search_method_using_and(self):
        parser = logfind.return_parser()
        parsed_arguments = parser.parse_args(self.arguments_and)
        assert_true(logfind.search_method(parsed_arguments, 'this is a test'))

    def test_search_method_using_and_fail(self):
        parser = logfind.return_parser()
        parsed_arguments = parser.parse_args(self.arguments_and)
        assert_false(logfind.search_method(parsed_arguments, 'this is a '))
